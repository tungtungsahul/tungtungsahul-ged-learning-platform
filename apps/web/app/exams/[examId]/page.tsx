...
export default function Exam(){
 ...
 useEffect(()=>{api<any>(`/exams/${examId}/start`,{method:"POST"}).then(x=>{setA(x);setRemaining(Math.max(0,Math.floor((new Date(x.expiresAt).getTime()-Date.now())/1000)));const m:any={};x.answers?.forEach((z:any)=>m[z.questionId]=z.selectedOptionId||z.textValue||z.payload);setAnswers(m);setEliminated(x.workspaceState?.eliminated||{})})},[examId]);
 ...
 async function saveWorkspace(state:any){if(!a)return;try{await api(`/exams/${examId}/attempts/${a.id}/workspace`,{method:"POST",body:JSON.stringify({state})})}catch(e:any){setMsg(e.message)}}
 ...
 return <div className="examShell">
  ...
  <div className="examBody">
   <section className="passagePane" style={{display:mode==="question"?"block":undefined}}>
     <div className="toolbar"><button className="btn btn-soft" onClick={()=>setHl(!hl)}>{hl?"Highlight on":"Highlight"}</button><button className="btn btn-muted" onClick={()=>{setEliminated({});saveWorkspace({eliminated:{}})}}>Clear eliminations</button><button className="btn btn-muted" onClick={()=>window.getSelection()?.removeAllRanges()}>Clear selection</button></div>
   ...
   <section className="questionPane" style={{display:mode==="passage"?"none":undefined}}>
    <div style={{maxWidth:720,margin:"auto"}}><SubjectWorkspace subject={a.exam.subject} passage={passage} question={q}/><div className="badge">Question {i+1} of {a.exam.questions.length} · {q.topic}</div><h2 style={{fontSize:26,lineHeight:1.4}}>{q.text}</h2>
      {(q.type==="MULTIPLE_CHOICE"||q.type==="DROPDOWN"||q.type==="TRUE_FALSE")&&q.options?.map((o:any)=><button key={o.id} className={`option ${answers[q.id]===o.id?"sel":""}`} onClick={()=>save(o.id)} style={{opacity:eliminated[o.id] ? .35:1}}><span className="optlabel">{o.label}</span><span>{o.text}</span><button onClick={(e)=>{e.stopPropagation();setEliminated(x=>{const next={...x,[o.id]:!x[o.id]};saveWorkspace({eliminated:next});return next})}} className="btn btn-muted" style={{marginLeft:"auto",padding:"6px 8px"}}>✕</button></button>)}
